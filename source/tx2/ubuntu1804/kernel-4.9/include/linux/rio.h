...
#define RIO_MAX_MSG_SIZE	0x1000   // 4096
...
#define RIO_OUTB_MBOX_RESOURCE	2              // rio_request_outb_mbox 里初始化
 ...
 */
// rio_setup_device 申请该结构
// rionet_setup_netdev
// rio_mport_add_riodev
struct rio_dev {
	struct list_head global_list;	/* node in list of all RIO devices */     // 挂到  rio_devices  里
	                                                                         //  rio_add_device  挂树
...
struct rio_msg {
	...
	void (*mcback) (struct rio_mport * mport, void *dev_id, int mbox, int slot);   
	                // rio_request_outb_mbox 里设置 到 rio_mport.outb_msg[1].mcback = riocm_outb_msg_event       然后在 tsi721_omsg_handler 里调用
	                // rio_request_inb_mbox 里设置到   rio_mport.    inb_msg[mbox].mcback = riocm_inb_msg_event  在 tsi721_imsg_handler 里调用
};
 ...
 */
struct rio_dbell {
	struct list_head node; // 挂到 rio_mport.dbells 里
	                       // rio_setup_inb_dbell 里挂树
	                       // tsi721_db_dpc 遍历树
	struct resource *res;
	void (*dinb) (struct rio_mport *mport, void *dev_id, u16 src, u16 dst, u16 info);  // tsi721_db_dpc 里调用
	                                                                                   // rio_mport_doorbell_handler 回调接口
	void *dev_id;
};
 ...
 */
//  rio_mport 是驱动管理设备的接口             对应的用户接口是 mport_dev 由 mport_cdev_add 创建
// 一个物理设备 rio_mport 对应一个 字符设备 mport_dev
//  tsi721_device.mport
struct rio_mport {
	struct list_head dbells;	/* list of doorbell events */    // rio_dbell.node 挂在这里
	                                                             // rio_setup_inb_dbell 里挂树
	                                                             // tsi721_db_dpc 遍历树
	struct list_head pwrites;	/* list of portwrite events */   // rio_pwrite.node 挂到这里
	                                                             // rio_add_mport_pw_handler 里挂树
	                                                             // rio_inb_pwrite_handler 里遍历 
	struct list_head node;	/* node in global list of ports */   // 挂到  rio_mports 里   rio_register_mport 里挂树
	struct list_head nnode;	/* node in net list of ports */
	struct rio_net *net;	/* RIO net this mport is attached to */
	struct mutex lock;
	struct resource iores;
	struct resource riores[RIO_MAX_MPORT_RESOURCES];   //  tsi721 : tsi721_setup_mport 里初始化
	                                                   // rio_request_inb_dbell 
	                                                   // rio_request_outb_mbox
	struct rio_msg inb_msg[RIO_MAX_MBOX];              //  inbound msg 接口
	                                                   // rio_request_inb_mbox 里注册本接口
	struct rio_msg outb_msg[RIO_MAX_MBOX];
	int host_deviceid;	/* Host device ID */   // 设备 id   rio_mport_initialize 里初始化
	                                        //  tsi721_open_inb_mbox 里写设备寄存器
	                                        // maint_hdid_set 可以通过字符设备接口进行修改
	struct rio_ops *ops;	/* low-level architecture-dependent routines */
	                    //  tsi721 : tsi721_rio_ops     tsi721_setup_mport 里设置
	unsigned char id;	/* port ID, unique among all ports */
	                //   系统里的一个唯一值               不超过 RIO_MAX_MPORTS     
	                //   rio_mport_initialize  里初始化
	                // 判断      rio_register_mport
	unsigned char index;	/* port index, unique among all port interfaces of the same type */
	                      // 初始化成 0       tsi721_setup_mport 里初始化
	unsigned int sys_size;	/* RapidIO common transport system size.
				 * 0 - Small size. 256 devices.
				 * 1 - Large size, 65536 devices.
				 */
	u32 phys_efptr;   // tsi721 值 : 0x100    tsi721_setup_mport
	u32 phys_rmap;    // tsi721 值 : 1   tsi721_setup_mport
	unsigned char name[RIO_MAX_MPORT_NAME];  //  tsi721  "tsi721(01:02.03)"      tsi721_setup_mport 里赋值
	struct device dev;
	void *priv;		/* Master port private data */     // tsi721   指向 tsi721_device       tsi721_setup_mport
#ifdef CONFIG_RAPIDIO_DMA_ENGINE
	struct dma_device	dma;
#endif
	struct rio_scan *nscan;
	atomic_t state;   //  RIO_DEVICE_INITIALIZING
	                 // rio_register_mport   RIO_DEVICE_RUNNING
 ...
 */
// rio_register_scan 申请本结构
struct rio_scan_node {
	int mport_id;
	struct list_head node;  // 加到 rio_scans 队列里
	                        // rio_register_scan 挂树
	struct rio_scan *ops;  // rio_scan_ops
};

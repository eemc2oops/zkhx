...
static LIST_HEAD(kthread_create_list);  // kthreadd 跟据链表创建内核线程
                                        // __kthread_create_on_node 向链接里添加待创建的内核线程
						    ...
						    va_list args)
{
	...
	list_add_tail(&create->list, &kthread_create_list);  // 查看 kthreadd
...
int kthreadd(void *unused)
{
	...
	for (;;) {
		...
		if (list_empty(&kthread_create_list))   // 查看 __kthread_create_on_node
			schedule();
		__set_current_state(TASK_RUNNING);

...
struct tegra_pcie {
	struct device *dev;   //  指向 platform_device    tegra_pcie_probe
	struct pci_host_bridge *host;

	void __iomem *pads;
	void __iomem *afi;
	int irq;  // tegra_pcie_get_resources 里赋值

	struct list_head buses;  //  tegra_pcie_bus.list 挂到这里， 可以遍历到所有的 pci 设备。
	                         //  tegra_pcie_add_bus 挂设备
	...
	struct resource busn;  // tegra_pcie_parse_dt

	struct {
		resource_size_t mem;
		resource_size_t io;  // tegra_pcie_parse_dt
	} offset;   // 表示  resource里mem和io字段的偏移位置          tegra_pcie_request_resources

	void __iomem *cfg_va_base;   // tegra_pcie_bus_alloc  里赋值
	                             // 并不是在根总线对应结构生成时赋的值，而是在扫描到下一级总线时，才给赋的值
	...
	struct list_head ports;  // tegra_pcie_port.list  挂在这个队列里
	                        //  tegra_pcie_parse_dt  函数里挂树
	u32 xbar_config;
	int num_ports;
	...
	struct pinctrl_state	*pex_io_dpd_en_state;   // tegra_pcie_probe
	struct pinctrl_state	*pex_io_dpd_dis_state;  // tegra_pcie_probe
...
// 从device tree里解析出来的pci配置
struct tegra_pcie_port {
	struct tegra_pcie *pcie;   // 指向所属的 tera_pcie 结构
	struct list_head list;   // 挂在 tegra_pcie.ports 下面
	                         //  tegra_pcie_parse_dt  里挂树
...
struct tegra_pcie_bus {
	struct list_head list;  // 挂到 tegra_pcie.buses 下面
	                        //  tegra_pcie_add_bus 挂设备
	unsigned int nr;   // 总线号
};
 ...
 */
// tegra_pcie_add_bus -> tegra_pcie_bus_alloc
							...
							unsigned int busnr)
{
	...
	if (!pcie->cfg_va_base) {   // 并不是在根总线对应结构生成时赋的值，而是在扫描到下一级总线时，才给赋的值
	                            // 从 tegra_pcie_conf_address 里看，0号总线不使用这个base值
					     ...
					     int where)
{
	...
	if (bus->number == 0) {   // 0号总线不使用 pcie->cfg_va_base 
...
// tegra_pcie_restore_device -> tegra_pcie_request_resources
...
// pci_alloc_child_bus -> tegra_pcie_add_bus
...
// 从  rpm_resume 流程里调用下去
/*
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884272] chenyichong tegra_pcie_restore_device 
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884309] CPU: 1 PID: 813 Comm: kworker/1:1 Not tainted 4.9.140-tegra #44
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884344] Hardware name: quill (DT)
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884401] Workqueue: events pcie_delayed_detect
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884445] Call trace:
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884499] [<ffffff800808bdb8>] dump_backtrace+0x0/0x170
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884534] [<ffffff800808c354>] show_stack+0x24/0x30
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884565] [<ffffff8008458420>] dump_stack+0x98/0xc0
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884598] [<ffffff80084ee170>] tegra_pcie_restore_device+0x48/0x8b8
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884635] [<ffffff800876161c>] pm_generic_runtime_resume+0x3c/0x58
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884694] [<ffffff800876ee88>] __genpd_runtime_resume+0x38/0xa0
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884727] [<ffffff80087712dc>] genpd_runtime_resume+0x94/0x1c0
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884760] [<ffffff8008763964>] __rpm_callback+0x74/0xa0
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884791] [<ffffff80087639c4>] rpm_callback+0x34/0x98
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884822] [<ffffff8008764e60>] rpm_resume+0x470/0x710
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884853] [<ffffff800876514c>] __pm_runtime_resume+0x4c/0x70
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884886] [<ffffff80084ed648>] pcie_delayed_detect+0x3d8/0xeb8
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884921] [<ffffff80080d472c>] process_one_work+0x1e4/0x4b0
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884961] [<ffffff80080d4a48>] worker_thread+0x50/0x4c8
Jun 19 10:59:01 zkhx-desktop kernel: [    0.884992] [<ffffff80080db5bc>] kthread+0xec/0xf0
Jun 19 10:59:01 zkhx-desktop kernel: [    0.885023] [<ffffff8008083850>] ret_from_fork+0x10/0x40
*/
...
// tegra_pcie_init -> tegra_pcie_power_on
...
// tegra_pcie_init -> tegra_pcie_get_resources
...
// tegra_pcie_check_ports -> tegra_pcie_port_disable
...
// tegra_pcie_restore_device -> tegra_pcie_check_ports
static void tegra_pcie_check_ports(struct tegra_pcie *pcie)
{
	struct tegra_pcie_port *port, *tmp;
...
// tegra_pcie_probe_complete -> tegra_pcie_init
...
// tegra_pcie_probe -> tegra_pcie_parse_dt
...
// pcie_delayed_detect -> tegra_pcie_probe_complete
...
// 在 tegra_pcie_probe 里创建的延时任务里被调用
static void pcie_delayed_detect(struct work_struct *work)
{
	...
	pcie = container_of(work, struct tegra_pcie, detect_delay.work);  // 值的相互关系参见 tegra_pcie_probe
...
#endif
	ret = tegra_pcie_probe_complete(pcie);  // pci_host_bridge 里的  tegra_pcie
...
// platform_drv_probe -> tegra_pcie_probe

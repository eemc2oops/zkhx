 ...
 */
// __unflatten_device_tree -> unflatten_dt_nodes
...
int __initdata dt_root_addr_cells;   // early_init_dt_scan_root dts里root的地址配置信息 tx2 里赋的值为2
int __initdata dt_root_size_cells;  //  early_init_dt_scan_root dts里root的地址配置信息 tx2 里赋的值为2

void *initial_boot_params;   // dtb文件的起始地址 device tree配置          early_init_dt_verify 赋值
 ...
 * unflatten the tree
 */
// early_init_dt_scan_nodes -> of_scan_flat_dt
 ...
 */
// of_flat_dt_get_machine_name -> of_get_flat_dt_prop
...
// dump_stack_set_arch_desc -> of_flat_dt_get_machine_name
...
/*
在dts文件里，chosen的配置如下，  bootargs表示启动配置命令行
chosen {
		board-has-eeprom;
...
本函数的作用，把 dts里配置的cmdline添加到 全局变量 boot_command_line 里
early_init_dt_scan_chosen的data参数就是boot_command_line
*/

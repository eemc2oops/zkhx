 ...
 */
// rio_add_mport_pw_handler 里创建本结构
struct rio_pwrite {
	struct list_head node;  // 挂到 rio_mport.pwrites 里
	                        // rio_add_mport_pw_handler 里挂树
                            // rio_inb_pwrite_handler 里遍历 
	int (*pwcback)(struct rio_mport *mport, void *context,
		       union rio_pw_msg *msg, int step);  // rio_inb_pwrite_handler 里调用
		                                         // rio_add_mport_pw_handler 里注册本结构时挂的接口是              rio_mport_pw_handler   
	void *context;  // 指向    mport_dev 
	                // rio_mport_add_pw_filter  里赋值
	                // rio_inb_pwrite_handler 里使用
};
...
//  insmod **.ko hdid=101,202,333,432,5657,888
static int hdid[RIO_MAX_MPORTS];   //  rio_get_hdid  里分配 rapidio 的 host id
static int ids_num;     // rio_get_hdid 里使用
module_param_array(hdid, int, &ids_num, 0);
MODULE_PARM_DESC(hdid,
	"Destination ID assignment to local RapidIO controllers");

// 方便看代码跳转  原始定义是             static LIST_HEAD(rio_devices)
static struct list_head rio_devices;
// static LIST_HEAD(rio_devices);  // rio_dev.global_list 挂到这里
                                //  rio_get_comptag 里遍历 
                                //  rio_add_device  挂树
static LIST_HEAD(rio_nets);
static DEFINE_SPINLOCK(rio_global_list_lock);

static struct list_head rio_mports;   // 方便跳转看代码  原始的定义是 static LIST_HEAD(rio_mports);
// static LIST_HEAD(rio_mports);  //   rio_mport.node 挂到这个链表里，  表示所有注册的 mport 
                                //   rio_register_mport  里挂链表 

static struct list_head rio_scans;   //  方便跳转看代码           static LIST_HEAD(rio_scans)
                                    // rio_register_scan 向队列里添加
// static LIST_HEAD(rio_scans);    //  rio_register_mport    里扫描
 ...
 */
// mport_cdev_add -> rio_query_mport
int rio_query_mport(struct rio_mport *port,
		    struct rio_mport_attr *mport_attr)
{
	if (!port->ops->query_mport)
		return -ENODATA;
	return port->ops->query_mport(port, mport_attr);  // tsi721 : tsi721_query_mport
}
 ...
 */
// rio_mport_add_riodev -> rio_alloc_net
 ...
 */
// maint_hdid_set -> rio_local_set_device_id
 ...
 * for an RIO device.
 */
// rio_setup_device -> rio_add_device
// rio_mport_add_riodev -> rio_add_device
 ...
 */
// riocm_add_mport -> rio_request_inb_mbox
				       ...
				       int slot))
{
	...
	if (res) {
		...
		rc = mport->ops->open_inb_mbox(mport, dev_id, mbox, entries);  // tsi721_open_inb_mbox
		if (rc) {
 ...
 * a callback function to the resource. Returns 0 on success.
 */
// riocm_add_mport -> rio_request_outb_mbox
			  ...
			  void (*moutb) (struct rio_mport * mport, void *dev_id, int mbox, int slot))
{
	...
	if (res) {
		...
		rc = mport->ops->open_outb_mbox(mport, dev_id, mbox, entries);  // tsi721 : tsi721_open_outb_mbox
		if (rc) {
 ...
 * satisfied.
 */
// rio_request_inb_dbell -> rio_setup_inb_dbell
static int
 ...
 * has been satisfied.
 */
// rio_mport_add_db_filter -> rio_request_inb_dbell
 ...
 */
// rio_mport_add_pw_filter -> rio_add_mport_pw_handler
			     ...
			     void *context, union rio_pw_msg *msg, int step))
{
	...
	pwrite->pwcback = pwcback;   // rio_inb_pwrite_handler 里调用
 ...
 */
// rio_mport_add_pw_filter -> rio_pw_enable
void rio_pw_enable(struct rio_mport *mport, int enable)
{
	if (mport->ops->pwenable) {
		...
		if ((enable && ++mport->pwe_refcnt == 1) ||
			...
			mport->ops->pwenable(mport, enable);  // tsi721_pw_enable
		mutex_unlock(&mport->lock);
	}
}
 ...
 * list.
 */
// rio_inb_pwrite_handler -> rio_get_comptag
 ...
 */
// rio_inb_pwrite_handler -> rio_chk_dev_route
static int
 ...
 * @rdev: Pointer to RIO device control structure
 */
// rio_inb_pwrite_handler -> rio_chk_dev_access
 ...
 * has been satisfied.
 */
// tsi721_pw_dpc -> rio_inb_pwrite_handler
int rio_inb_pwrite_handler(struct rio_mport *mport, union rio_pw_msg *pw_msg)
{
	 ...
	 */
	if (rdev && rdev->pwcback) {  // rdev 是 rio_net 的流程，当前可以不看
	...
	mutex_lock(&mport->lock);
	list_for_each_entry(pwrite, &mport->pwrites, node)
		pwrite->pwcback(mport, pwrite->context, pw_msg, 0);   // rio_mport_pw_handler 过滤接口              这个接口通过字符设备的 ioctl 接口配置
	mutex_unlock(&mport->lock);
...
// tsi721_setup_mport -> rio_mport_initialize
...
// tsi721_setup_mport -> rio_register_mport
int rio_register_mport(struct rio_mport *port)
{
	...
	port->dev.class = &rio_mport_class;    // 设备会挂到 rio_register_mport 下面

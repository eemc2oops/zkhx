 ...
 * @comp:
 */
// 这个是跟据 驱动里的 rio_mport 创建的一个字符设备，用作用户接口
// 一个字符设备 mport_dev 对应一个 物理设备 rio_mport
// mport_cdev_add 创建 mport_dev   
struct mport_dev {
	atomic_t		active;
	struct list_head	node;   // 挂到   mport_devs 队列里
	                            // mport_cdev_add  里挂队列
	struct cdev		cdev;
	struct device		dev;
	struct rio_mport	*mport;   //  一个 字符设备 mport_dev 对应一个物理设备 rio_mport
	                              //  mport_cdev_add 里赋值  
	...
	spinlock_t			db_lock;
	struct list_head		portwrites;    // rio_mport_pw_filter.md_node 挂在这个队列里
 ...
 */
// 本结构属于 file.private_data  每打开一次字符设备，会生成对应的本结构
// mport_cdev_open 里申请本结构，用来链接字符设备的标识 字符设备句柄 file 和 mport_dev 的关系
// 字符设备可以被打开多次，但是设备只会有一个， 可能存在多个 file 对应一个 mport_dev 
struct mport_cdev_priv {
	struct mport_dev	*md;   // 指向对应的 mport_dev
	struct fasync_struct	*async_queue;
	struct list_head	list;   // 挂在 mport_dev.file_list 队列里       mport_cdev_open 函数挂队列
	struct list_head	db_filters;   // rio_mport_db_filter.priv_node  挂在这里
	                                  // rio_mport_add_db_filter 里挂树
	struct list_head        pw_filters;   //  rio_mport_pw_filter.priv_node  挂在这个队列里
	                                       //  rio_mport_add_pw_filter 挂树 
	struct kfifo            event_fifo;    // mport_cdev_open 里申请
	                                       // 这个是收包处理流程            rio_mport_add_event  向队列里添加数据
	                                       // 用户读流程       mport_read  从队列里取数据
	wait_queue_head_t       event_rx_wait;  // mport_cdev_open 里初始化
	                                        // 收包处理流程       rio_mport_add_event 里  唤醒本队列
	                                        // 用户读 mport_read     里等待本队列 
	spinlock_t              fifo_lock;
	u32			event_mask; /* RIO_DOORBELL, RIO_PORTWRITE */    // mport_cdev_ioctl ( RIO_SET_EVENT_MASK ) 里赋值
 ...
 */
// rio_mport_add_pw_filter 里申请该结构
struct rio_mport_pw_filter {
	struct list_head md_node;  // 挂在 mport_dev.portwrites 队列里
	                            // rio_mport_add_pw_filter 挂树
	                          //  rio_mport_pw_handler 遍历取出本结构
	struct list_head priv_node;   // 挂在 mport_cdev_priv.pw_filters 队列里
	                              // rio_mport_add_pw_filter 挂树
	struct mport_cdev_priv *priv;   // rio_mport_add_pw_filter 里赋值
	struct rio_pw_filter filter;  // rio_mport_pw_handler 里判断
};
 ...
 */
// rio_mport_doorbell_handler 遍历到本结构
struct rio_mport_db_filter {
	struct list_head data_node; // 挂到 mport_dev.doorbells 队列里
	                            // rio_mport_add_db_filter 里挂树
	struct list_head priv_node;  // 挂到 mport_cdev_priv.db_filters 队列里
	                            // rio_mport_add_db_filter 里挂树
	struct mport_cdev_priv *priv;
	struct rio_doorbell_filter filter;
};

// 为了方便读代码时跳转，改成变量直接定义，正确的写法是 static LIST_HEAD(mport_devs)
static struct list_head mport_devs;  // mport_dev.node  挂到这个队列里         
                                     // mport_cdev_add 函数里挂队列
...
static struct class *dev_class;   // mport_init  里注册 class 的操作接口
static dev_t dev_number;   // mport_init  驱动加载时申请主设备号
                           // mport_cdev_add 由用户接口调用，创建真正的字符设备

static struct workqueue_struct *dma_wq;   // 创建一个工作队列  mport_init 里创建

static void mport_release_mapping(struct kref *ref);
// mport_cdev_ioctl ( RIO_MPORT_MAINT_READ_LOCAL ) -> rio_mport_maint_rd
// mport_cdev_ioctl ( RIO_MPORT_MAINT_READ_REMOTE ) -> rio_mport_maint_rd
...
// mport_cdev_ioctl ( RIO_MPORT_MAINT_WRITE_LOCAL ) -> rio_mport_maint_wr
// mport_cdev_ioctl ( RIO_MPORT_MAINT_WRITE_REMOTE ) -> rio_mport_maint_wr
 ...
 */
// mport_cdev_ioctl ( RIO_UNMAP_OUTBOUND ) -> rio_mport_obw_free
 ...
 * @arg:	Device Id
 */
// mport_cdev_ioctl ( RIO_MPORT_MAINT_HDID_SET ) -> maint_hdid_set
 ...
 * @arg:	Component Tag
 */
// mport_cdev_ioctl ( RIO_MPORT_MAINT_COMPTAG_SET ) -> maint_comptag_set
 ...
 */
// rio_dma_transfer -> do_dma_request
...
// mport_cdev_ioctl ( RIO_TRANSFER ) -> rio_mport_transfer_ioctl
...
// mport_cdev_ioctl ( RIO_WAIT_FOR_ASYNC ) -> rio_mport_wait_for_async_dma
...
// mport_cdev_ioctl ( RIO_FREE_DMA ) -> rio_mport_free_dma
 ...
 * @arg:  buffer handle returned by allocation routine
 */
// mport_cdev_ioctl ( RIO_UNMAP_INBOUND ) -> rio_mport_inbound_free
 ...
 * @arg:  port index
 */
// mport_cdev_ioctl ( RIO_MPORT_MAINT_PORT_IDX_GET ) -> maint_port_idx_get
...
// rio_mport_pw_handler -> rio_mport_add_event      tsi721 中断触发下半部处理流程
// rio_mport_doorbell_handler -> rio_mport_add_event   中断下半部 接收 doorbell 消息的流程
...
// tsi721_db_dpc -> rio_mport_doorbell_handler
...
// mport_cdev_ioctl ( RIO_ENABLE_DOORBELL_RANGE ) -> rio_mport_add_db_filter
...
// mport_cdev_ioctl ( RIO_DISABLE_DOORBELL_RANGE ) -> rio_mport_remove_db_filter
...
// rio_mport_pw_handler -> rio_mport_match_pw
...
// rio_mport_add_pw_filter 里挂到过滤队列里
// rio_inb_pwrite_handler -> rio_mport_pw_handler     tsi721 port write 中断触发下半部处理流程
...
// mport_cdev_ioctl ( RIO_ENABLE_PORTWRITE_RANGE ) -> rio_mport_add_pw_filter
...
// mport_cdev_ioctl ( RIO_DISABLE_PORTWRITE_RANGE ) -> rio_mport_remove_pw_filter
 ...
 */
// mport_cdev_ioctl ( RIO_DEV_ADD ) -> rio_mport_add_riodev
...
// mport_cdev_ioctl ( RIO_DEV_DEL ) -> rio_mport_add_riodev
...
// mport_cdev_add 里注册字符设备时， 设备的回调函数
 ...
 */
// mport_add_mport -> mport_cdev_add
static struct mport_dev *mport_cdev_add(struct rio_mport *mport)
{
	...
	md->dev.class = dev_class;   // dev_class 是在 mport_init 里注册的 class 操作接口  class 名字是 "rio_mport"
	md->dev.parent = &mport->dev;
	md->dev.release = mport_device_release;
	dev_set_name(&md->dev, DEV_NAME "%d", mport->id);  //  DEV_NAME : "rio_mport"
 ...
 */
// 接口挂在 rio_mport_interface 里， 是在 mport_init 里注册到系统里的
// class_interface_register -> mport_add_mport
// device_add -> mport_add_mport
static int mport_add_mport(struct device *dev,
		struct class_interface *class_intf)
{
	struct rio_mport *mport = NULL;  // tsi721 驱动里创建的设备结构
...
// mport_add_mport 接口挂在这个 class 下面
...
static int __init mport_init(void)
{
	...
	dev_class = class_create(THIS_MODULE, DRV_NAME);  // DRV_NAME : "rio_mport"
	if (IS_ERR(dev_class)) {
		rmcd_error("Unable to create " DRV_NAME " class");
		return PTR_ERR(dev_class);
	}

	ret = alloc_chrdev_region(&dev_number, 0, RIO_MAX_MPORTS, DRV_NAME);   // DRV_NAME : "rio_mport"
	if (ret < 0)                                                          //  申请字符设备的主设备号
		goto err_chr;

	rmcd_debug(INIT, "Registered class with major=%d", MAJOR(dev_number));

	/* Register to rio_mport_interface */
	ret = class_interface_register(&rio_mport_interface);   // 注册用户接口
	if (ret) {

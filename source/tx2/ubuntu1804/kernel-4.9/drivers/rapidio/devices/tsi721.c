 ...
 */
// 读tsi721的配置空间bar0里的设备寄存器
// rio_mport_maint_rd -> __rio_local_read_config_32 -> tsi721_lcread
 ...
 * success or %-EINVAL on failure.
 */
// rio_mport_maint_wr -> __rio_local_write_config_32 -> tsi721_lcwrite
 ...
 */
// tsi721_cread_dma -> tsi721_maint_dma
// tsi721_cwrite_dma -> tsi721_maint_dma
 ...
 * Returns %0 on success and %-EINVAL or %-EFAULT on failure.
 */
// rio_mport_maint_rd -> rio_mport_read_config_32 -> tsi721_cread_dma
 ...
 * Returns %0 on success and %-EINVAL or %-EFAULT on failure.
 */
// rio_mport_maint_wr -> rio_mport_write_config_32 -> tsi721_cwrite_dma
 ...
 * queued messages.
 */
// tsi721_srio_msix -> tsi721_pw_handler
static int
...
// tsi721_port_write_init 里用 INIT_WORK 初始化到    tsi721_device.pw_work  里
// 中断处理 tsi721_pw_handler 里唤醒本工作队列
// port write 中断的延时操作
 ...
 */
// rio_pw_enable -> tsi721_pw_enable
 ...
 */
// rio_mport_send_doorbell -> tsi721_dsend
 ...
 * queued DBs.
 */
// tsi721_sr2pc_ch_msix -> tsi721_dbell_handler     msix 中断流程
// tsi721_irqhandler -> tsi721_dbell_handler  intx msi 中断流程
static int
...
// 在 tsi721_doorbell_init 里注册    doorbell中断的延时处理
		...
		*idb_entry = 0;
		...
		if (found) {
			dbell->dinb(mport, dbell->dev_id, DBELL_SID(idb.bytes),
				    DBELL_TID(idb.bytes), DBELL_INF(idb.bytes));   // rio_mport_doorbell_handler
		} else {
			tsi_debug(DBELL, &priv->pdev->dev,
 ...
 */
// tsi721_request_irq 里设置该中断处理函数   INTx 方式的中断
...
// tsi721_probe -> tsi721_interrupts_init
 ...
 */
// tsi721_request_msix 里设置该中断处理函数
...
static irqreturn_t tsi721_sr2pc_ch_msix(int irq, void *ptr)
{
	...
	if (sr_ch_int & TSI721_SR_CHINT_IDBQRCV)
		tsi721_dbell_handler(priv); // This inbound doorbell queue sent a new inbound doorbell since this bit was last cleared.
 ...
 */
// tsi721_request_irq -> tsi721_request_msix
 ...
 */
// tsi721_setup_mport -> tsi721_enable_msix
...
// tsi721_setup_mport -> tsi721_request_irq
 ...
 */
// tsi721_probe -> tsi721_init_pc2sr_mapping
 ...
 */
// tsi721_probe -> tsi721_init_sr2pc_mapping
 ...
 */
// tsi721_probe -> tsi721_port_write_init
...
// tsi721_probe -> tsi721_doorbell_init
static int tsi721_doorbell_init(struct tsi721_device *priv)   // 只使用了 dbqueue 0 其它1~7个queue没有用
{
	...
	INIT_WORK(&priv->idb_work, tsi721_db_dpc);   // 注册中断的下半部处理

	/* Allocate buffer for inbound doorbells queue */
	priv->idb_base = dma_zalloc_coherent(&priv->pdev->dev,    // 申请 DMA 内存
 ...
 * Returns %0 on success or %-ENOMEM on failure.
 */
// tsi721_probe -> tsi721_bdma_maint_init
static int tsi721_bdma_maint_init(struct tsi721_device *priv)
{
	...
	sts_size = (bd_num >= TSI721_DMA_MINSTSSZ) ?
					bd_num : TSI721_DMA_MINSTSSZ;  // 32
	sts_size = roundup_pow_of_two(sts_size); // 32
	...
	iowrite32(((u64)bd_phys >> 32),	regs + TSI721_DMAC_DPTRH);   // 0x51028
	...
	iowrite32(TSI721_DMAC_DSSZ_SIZE(sts_size),  //  TSI721_DMAC_DSSZ_SIZE(sts_size) = 1
		regs + TSI721_DMAC_DSSZ);   //  0x51034

	/* Clear interrupt bits */
	iowrite32(TSI721_DMAC_INT_ALL, regs + TSI721_DMAC_INT); // 0x5100c
...
// tsi721_open_outb_mbox -> tsi721_omsg_interrupt_enable
static void
 ...
 */
// rio_request_outb_mbox -> tsi721_open_outb_mbox
 ...
 */
// tsi721_irqhandler -> tsi721_imsg_handler      intx  msi 中断方式
// tsi721_imsg_msix -> tsi721_imsg_handler     msix 中断方式
static void tsi721_imsg_handler(struct tsi721_device *priv, int ch)
{
	u32 mbox = ch - 4;
	u32 imsg_int;
	...
	if (imsg_int & TSI721_IBDMAC_INT_DQ_RCV &&
		...
		mport->inb_msg[mbox].mcback(mport,
				priv->imsg_ring[mbox].dev_id, mbox, -1);    //  riocm_inb_msg_event
 ...
 */
// rio_request_inb_mbox -> tsi721_open_inb_mbox
 ...
 */
// rio_add_inb_buffer -> tsi721_add_inb_buffer
 ...
 */
// rio_get_inb_message -> tsi721_get_inb_message
 ...
 */
// tsi721_probe -> tsi721_messages_init
 ...
 * @priv: pointer to tsi721 private data
 */
// tsi721_probe -> tsi721_disable_ints
...
// tsi721_setup_mport 里设置给 rio_mport.ops
 ...
 */
//  tsi721_probe -> tsi721_setup_mport
static int tsi721_setup_mport(struct tsi721_device *priv)
{
	...
	err = tsi721_request_irq(priv); // 挂中断处理函数
				  ...
				  const struct pci_device_id *id)
{
	...
	err = tsi721_doorbell_init(priv);  // 注册 INIT_WORK
	if (err)
		goto err_free_bdma;

	tsi721_port_write_init(priv);   // 注册 INIT_WORK
    ...
    // 然后设置中断，DMA等
	err = tsi721_setup_mport(priv);  //  挂中断处理函数          中断处理函数        tsi721_sr2pc_ch_msix  tsi721_srio_msix
	if (err)
		goto err_free_consistent;
...
module_pci_driver(tsi721_driver);  // module_pci_driver 定义在 include/linux/pci.h 里   initcall 6

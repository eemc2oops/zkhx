...
static int cmbox = 1;     // mailbox num  用户加模块时指定
...
// cm_dev 对应每个设备             一个 tsi721 对应一个         cm_dev
// riocm_add_mport 里创建本结构
struct cm_dev {
	struct list_head	list;   // 挂在 cm_dev_list 队列里
	                            //  riocm_add_mport 里挂树
	struct rio_mport	*mport;
	void			*rx_buf[RIOCM_RX_RING_SIZE];  // 每个元素指向 4096 大小的地址空间
	                                              // riocm_rx_fill 申请 
	                                              // riocm_rx_get_msg
	                                              // 指向的是 tsi721_imsg_ring.imq_base 里的缓存地址  
	int			rx_slots;
	...
	u32			npeers;
	struct workqueue_struct *rx_wq;  // riocm_add_mport 里创建
	struct work_struct	rx_work;  //  rx_work.func = rio_ibmsg_handler
	                              // // riocm_add_mport 里赋值
};
// rio_channel.rx_ring
struct chan_rx_ring {
	void	*buf[RIOCM_RX_RING_SIZE];   // 收包缓冲
	                                   // rio_rx_data_handler  中断收到包，把包放到缓冲队列里
	                                  //  riocm_ch_receive  用户收包
	int	head;
	int	tail;
	int	count;
...
// riocm_cdev_add
struct channel_dev {
	struct cdev	cdev;    // 标识 rio_cm 字符设备
	struct device	*dev;   // 动态创建的设备
};
...
// 方便阅读代码添加，   正常写法             static DEFINE_IDR(ch_idr);
static struct idr ch_idr;     //
                              //  riocm_ch_alloc 里申请
// static DEFINE_IDR(ch_idr);

// 为了阅读代码方便，正确的写法是                     static LIST_HEAD(cm_dev_list)
static struct list_head cm_dev_list;    // cm_dev.list 挂在这个队列里
                                        //  riocm_add_mport 里挂树
// static LIST_HEAD(cm_dev_list);
static DECLARE_RWSEM(rdev_sem);

static struct class *dev_class;     // riocm_init 里创建class
                                    // riocm_cdev_add 往 class 里添加设备
static unsigned int dev_major;
static unsigned int dev_minor_base;
static dev_t dev_number;
static struct channel_dev riocm_cdev;   // 字符设备
                                        // riocm_cdev_add 里初始化
...
// rio_rx_data_handler -> riocm_get_channel       inbound mailbox 中断延时处理流程
// cm_chan_msg_rcv -> riocm_get_channel           inbound mailbox  用户收包流程
...
// rio_ibmsg_handler -> riocm_rx_get_msg
static void *riocm_rx_get_msg(struct cm_dev *cm)
{
	void *msg;
	int i;

	msg = rio_get_inb_message(cm->mport, cmbox);  // msg 用的是预先分配好的缓存 cm->rx_buf 里预先分配的内存
	if (msg) {
 ...
 * Returns: none
 */
// riocm_add_mport -> riocm_rx_fill
// rio_ibmsg_handler -> riocm_rx_fill     收 mailbox 消息流程
 ...
 */
// rio_ibmsg_handler -> rio_rx_data_handler
...
static void rio_ibmsg_handler(struct work_struct *work)
{
	...
	while (1) {
		...
		if (data)
			riocm_rx_fill(cm, 1);  //  取了一块内存，申请一块内存补回去
...
// tsi721_imsg_handler -> riocm_inb_msg_event
				...
				int mbox, int slot)
{
	...
	if (rio_mport_is_running(cm->mport) && !work_pending(&cm->rx_work))
		queue_work(cm->rx_wq, &cm->rx_work);   // 激活 rio_ibmsg_handler
}
...
// cm_chan_msg_rcv -> riocm_ch_free_rxbuf
 ...
 */
// cm_chan_msg_rcv -> riocm_ch_receive
 ...
 */
// cm_chan_accept -> riocm_ch_accept
 ...
 */
// riocm_ch_accept -> riocm_ch_alloc
// riocm_ch_create -> riocm_ch_alloc
static struct rio_channel *riocm_ch_alloc(u16 ch_num)
{
	int id;
	int start, end;
 ...
 */
// cm_chan_create -> riocm_ch_create
 ...
 */
// riocm_cdev_ioctl ( RIO_CM_CHAN_CREATE ) -> cm_chan_create
 ...
 * @arg:	Channel number
 */
// riocm_cdev_ioctl ( RIO_CM_CHAN_ACCEPT ) -> cm_chan_accept
 ...
 */
// riocm_cdev_ioctl ( RIO_CM_CHAN_RECEIVE ) -> cm_chan_msg_rcv
...
static int riocm_cdev_add(dev_t devno)
{
	...
	riocm_cdev.dev = device_create(dev_class, NULL, devno, NULL, DEV_NAME);  // DEV_NAME        "rio_cm"
 ...
 */
// sys fs 的 add_dev 接口
// class_interface_register -> riocm_add_mport
...
static int __init riocm_init(void)
{
	...
	dev_class = class_create(THIS_MODULE, DRV_NAME);  // DRV_NAME        "rio_cm"
	                                                 // riocm_cdev_add 里 注册了一个设备到这个class上
	if (IS_ERR(dev_class)) {
